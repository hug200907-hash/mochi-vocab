...
# ============================================================
# 6. TÍNH THỜI ĐIỂM VÀNG & CẤP ĐỘ
# ============================================================

def calculate_golden_interval(item, is_correct, response_time):
    old_interval = float(item.get("interval", 5))
    ...
    if not is_correct:
        new_interval = old_interval * WRONG_FACTOR
        new_interval = max(MIN_INTERVAL, new_interval)
        return int(round(new_interval))

    level_factor = LEVEL_FACTORS.get(old_level, 1.0)
    speed_factor = calculate_speed_factor(response_time)
    accuracy_factor = calculate_accuracy_factor(item)

    base_interval = 10 if item.get("review_count", 0) == 0 else old_interval
...
def calculate_level(old_level, is_correct, response_time):
    old_level = max(0, min(old_level, 5))
    if is_correct:
        increase = 1 if response_time <= NORMAL else 0
        new_level = min(5, old_level + increase)
    else:
    ...
    if saved_data:
        try:
            ...
            for item in items:
                ...
                item["level"] = max(0, min(int(item.get("level", 0)), 5))
                if "interval" not in item:
                    remaining = (item["next_review"] - datetime.now()).total_seconds() / 60
                    item["interval"] = max(MIN_INTERVAL, min(MAX_INTERVAL, remaining))
...
# ============================================================
# 8. TIỆN ÍCH PHÁT ÂM, DỊCH & TRA TỪ
# ============================================================
                ...
                for meaning in first.get("meanings", []):
                    ...
                    for definition in meaning.get("definitions", []):
                        if definition.get("definition"):
                            meanings_raw.append({"type": pos, "en": definition["definition"]})
...
def get_next_id():
    if not st.session_state.deck:
        return 1
    return max(int(x.get("id", 0)) for x in st.session_state.deck) + 1
...
def process_answer(is_correct, correct_ans_text):
    ...
    new_level = calculate_level(old_level, is_correct, response_time)
    new_interval = calculate_golden_interval(item, is_correct, response_time)
    ...
    item["last_result"] = "correct" if is_correct else "wrong"
    item["next_review"] = datetime.now() + timedelta(minutes=new_interval)

    if is_correct:
        st.success("✨ Chính xác!")
        st.write(f"⚡ Thời gian phản hồi: **{response_time:.1f} giây**")
        if new_level > old_level:
            st.success(f"📈 Cấp độ: {old_level} → {new_level}")
        else:
            st.info(f"📊 Cấp độ vẫn là **Cấp {new_level}**")

        st.info(f"🧠 Thời Điểm Vàng được tính lại: **{format_interval(new_interval)}**")
        if new_level == 5:
            st.balloons()
            st.success("🏆 Từ này đã đạt Cấp 5!")
    else:
        st.error("❌ Chưa chính xác.")
        st.warning(f"Đáp án đúng: **{correct_ans_text}**")
        st.warning(f"📉 Cấp độ: {old_level} → {new_level}")
        st.info(f"🔄 Thời Điểm Vàng mới: **{format_interval(new_interval)}**")
...
def prepare_review_question(item):
    ...
    if not example:
        ...
        example = online_example if online_example else f"The word '{word}' is very important."

    deck_words = [
        x.get("word", "").strip() for x in st.session_state.get("deck", [])
        if x.get("word", "").strip().lower() != word.lower() and x.get("word", "").strip()
    ]
    deck_meanings = [
        x.get("meaning", "").strip() for x in st.session_state.get("deck", [])
        if x.get("meaning", "").strip() != meaning and x.get("meaning", "").strip()
    ]
    ...
    if chosen_q == "CHOICE_MEANING":
        ...
        fallback_meanings = ["Sự phát triển", "Khả năng thích nghi", "Thành tựu", "Môi trường", "Kinh nghiệm"]
    ...
    # 2. CÂU -> ĐIỀN TỪ
        ...
        blank_sentence = re.sub(re.escape(word), "_____", example, flags=re.IGNORECASE)
    ...
    # 6. NGHE -> CHỌN TỪ & 7. NGHĨA -> CHỌN TỪ
...
st.title("🍌 MochiVocab")
st.caption("Thời Điểm Vàng được tính động theo quá trình ghi nhớ")

now = datetime.now()
due_count = sum(1 for x in st.session_state.deck if x["next_review"] <= now)
...
if selected_tab == "⏰ Ôn Tập":
    ...
    if not st.session_state.deck:
        ...
        with col2:
            st.metric("Cấp độ", next_item["level"])
        ...
        <div style="text-align:center; background:#262730; color:#00FF66; padding:20px; border-radius:15px; margin-top:15px;">
            ...
            <div id="countdown" style="font-size:30px; font-weight:bold; font-family:monospace;">00:00:00</div>
        </div>
        <script>
        ...
        function updateCountdown() {{
            ...
            if (diff <= 0) {{
                element.innerHTML = "🎉 ĐÃ ĐẾN GIỜ VÀNG!";
                return;
            }}
            ...
            if (days > 0) {{
                element.innerHTML = days + " ngày " + hours + ":" + minutes + ":" + seconds;
            }} else {{
        ...
        if not st.session_state.review_started:
            ...
            st.markdown(
                """
                ### 🧠 Sẵn sàng ôn tập?
                Khi bạn bấm **Bắt đầu ôn tập**, MochiVocab sẽ chọn từ cần ôn và bắt đầu tính thời gian phản hồi.
                """
            )
            if st.button("▶️ BẮT ĐẦU ÔN TẬP", type="primary", use_container_width=True, key="start_review"):
                min_level = min(x["level"] for x in due_items)
                candidates = [x for x in due_items if x["level"] == min_level]
            ...
            with col2:
                st.caption(f"Đã ôn: {item.get('review_count', 0)} lần")

            current_interval = item.get("interval", 5)
            st.caption(f"📐 Khoảng ôn hiện tại: **{format_interval(current_interval)}**")
            st.markdown("---")

            # -------------------------------------------------
            # HIỂN THỊ UI CHO CẢ 7 DẠNG CÂU HỎI
            # -------------------------------------------------
            ...
            if q_type == "CHOICE_MEANING":
                ...
                for index, option in enumerate(q_data.get("options", [])):
                    if st.button(option, key=f"choice_{item['id']}_{index}"):
                        process_answer(option == item["meaning"], item["meaning"])

            # DẠNG 2: FILL_BLANK
            elif q_type == "FILL_BLANK":
                st.markdown("### ✏️ ĐIỀN TỪ VÀO CÂU")
            ...
            # DẠNG 6: AUDIO_CHOICE
            elif q_type == "AUDIO_CHOICE":
                st.markdown("### 🔊 NGHE VÀ CHỌN TỪ ĐÚNG")
                st.info("Bấm nút nghe bên dưới và chọn từ tiếng Anh tương ứng:")
                if st.button("🔊 Nghe phát âm", type="primary", key=f"audio_play_{item['id']}"):
                    play_audio_script(item["word"])

                for index, option in enumerate(q_data.get("options", [])):
                    if st.button(option.upper(), key=f"audio_opt_{item['id']}_{index}"):
                        process_answer(option.lower() == item["word"].lower(), item["word"].upper())

            # DẠNG 7: MEANING_CHOICE
            elif q_type == "MEANING_CHOICE":
                st.markdown("### 🔤 NGHĨA -> CHỌN TỪ TIẾNG ANH")
                st.info(f"Nghĩa: **{q_data.get('question', '').upper()}**")
                st.write("Chọn từ tiếng Anh có nghĩa tương ứng:")
                for index, option in enumerate(q_data.get("options", [])):
                    if st.button(option.upper(), key=f"mchoice_opt_{item['id']}_{index}"):
                        process_answer(option.lower() == item["word"].lower(), item["word"].upper())
...
# ============================================================
    ...
    if data is not None and data["word"] == word_input:
        st.markdown("---")
        ...
        with col2:
            if st.button("➕ Thêm vào Sổ Tay", key="add_new_word"):
                exists = any(x["word"] == data["word"] for x in st.session_state.deck)
                if exists:
                    ...
                    st.info("⏰ Từ mới sẽ được ôn ngay lần đầu.")
                    time.sleep(1)
                    st.rerun()
...
# ============================================================
    ...
    if st.session_state.deck:
        ...
        due = sum(1 for x in st.session_state.deck if x["next_review"] <= datetime.now())
        mastered = sum(1 for x in st.session_state.deck if x["level"] == 5)
        ...
        with col3:
            st.metric("Cấp 5", mastered)

        st.markdown("---")

        table_data = []
        for item in st.session_state.deck:
            remaining = (item["next_review"] - datetime.now()).total_seconds()
            status = "🔥 Sẵn sàng ôn!" if remaining <= 0 else f"⏳ {format_remaining(remaining)}"

            accuracy_total = item.get("correct_count", 0) + item.get("wrong_count", 0)
            accuracy_text = f"{(item.get('correct_count', 0) / accuracy_total * 100):.0f}%" if accuracy_total > 0 else "—"

            table_data.append({
                "Từ": item["word"].upper(),
                "Nghĩa": item["meaning"],
                "Cấp": item["level"],
                "Trạng thái": get_level_name(item["level"]),
                "Khoảng ôn": format_interval(item.get("interval", 5)),
